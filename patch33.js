/* LIKE Estoque V33 - login com cores do sistema */
(function(){
  ...
  function applySystemLoginColors(){
    ...
    s.textContent=`
      ...
      body:not(.logged) #p-login:before{
        content:'';
        position:absolute;
        inset:0;
        background:linear-gradient(120deg,#ffffff08,#0f172a12);
      }
      body:not(.logged) #p-login .card{
        position:relative;
      ...
      body:not(.logged) #p-login .card h2{
        text-align:center;
        font-size:31px;
        margin:0 0 6px;
        color:#0d1b2e;
      ...
      body:not(.logged) #p-login .card p{
        text-align:center;
        color:#64748b;
        margin-bottom:22px;
      }
      body:not(.logged) #p-login .card:before{
        content:'👤';
        display:flex;
        align-items:center;
        justify-content:center;
        ...
        border-radius:999px;
        margin:0 auto 22px;
        background:linear-gradient(145deg,#0d1b2e,#0f4c81);
        color:white;
        font-size:58px;
        box-shadow:0 18px 45px #0f172a35;
      }
      body:not(.logged) #p-login .grid{display:block!important}
      body:not(.logged) #p-login input:not([type='hidden']){
        background:#f8fafc;
      ...
      body:not(.logged) #p-login input:not([type='hidden']):focus{
        border-color:#0f4c81;
        box-shadow:0 0 0 4px #0f4c8120;
      }
      body:not(.logged) #p-login input::placeholder{color:#64748b}
      body:not(.logged) #login{
        display:block;
        width:100%;
        margin-top:22px;
        padding:17px;
        ...
        font-size:16px;
        box-shadow:0 14px 34px #0f4c8138;
      }
      body:not(.logged) #login:hover{background:#173051}
      body:not(.logged) #loginMsg{
        background:#f8fafc;
        border-color:#e5e7eb;
        color:#172033;
        text-align:center;
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    setTimeout(applySystemLoginColors,400);
    setTimeout(applySystemLoginColors,1400);
  });
})();