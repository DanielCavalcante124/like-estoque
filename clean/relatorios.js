...
function renderTable(){
  ...
  $('relOut').innerHTML = `<div class="table-wrap rel-table-wrap"><table class="rel-table"><thead><tr>${d.head.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('') || `<tr><td colspan="${d.head.length}">Sem dados.</td></tr>`}</tbody></table></div>`;
