...
function setAdvanced(open){const box=$('loginAdvanced');if(!box)return;box.classList.toggle('open',!!open)}
function updateConfigStatus(){const c=cfg();const el=$('loginConfigStatus');if(!el)return;if(c.url&&c.key){el.textContent='Configuração salva';el.className='login-pill ok';setAdvanced(false)}else{el.textContent='Configuração pendente';el.className='login-pill bad';setAdvanced(true)}}
...
function bindLoginUi(){
 ...
 if($('btnClearConfig'))$('btnClearConfig').onclick=()=>{if(!confirm('Limpar URL/chave salvas neste navegador?'))return;localStorage.removeItem('like_cfg_v26');if($('loginUrl'))$('loginUrl').value='';if($('loginKey'))$('loginKey').value='';updateConfigStatus();setMsg('Configuração local apagada.','warn')};
