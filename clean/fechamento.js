...
const S = { preview:null, ultimo:null, historico:[] };
  ...
  if(!$('page-fechamento-clean')){
    ...
    sec.innerHTML = `
      <div class="card">
        ...
        <div class="fec-actions"><button id="fecPreview" class="primary">Gerar prévia</button><button id="fecSalvar" class="warn">Confirmar fechamento</button><button id="fecPdf" class="secondary">Baixar PDF jsPDF</button><button id="fecWhats" class="secondary">Copiar resumo WhatsApp</button></div>
        <div id="fecMsg" class="msg show">Gere uma prévia antes de confirmar o fechamento.</div>
      </div>
...
function validarCampos(){
  ...
  if(responsavel.length < 3) throw new Error('Informe o responsável pelo fechamento.');
...
async function gerarPreview(){
  try{
    ...
    S.preview = {
      ...
      payload:{relatorio:rel,auditoria:aud}
    };
    S.ultimo = null;
...
function saldoFinalFromRel(rel){
  const k = rel?.kpis || {};
  return {
    equipamentos_ativos:k.equipamentos_total || 0,
    em_estoque:k.em_estoque || 0,
    com_tecnico:k.com_tecnico || 0,
    manutencao:k.manutencao || 0,
    baixados_inativos:k.baixados_inativos || 0,
    valor_ativo:k.valor_total_ativo || 0,
    por_status:rel?.por_status || []
  };
}
async function salvarFechamento(){
  try{
    ...
    if(!confirm(`Confirmar fechamento de ${p.ini} até ${p.fim}? Essa ação salva histórico no banco.`)) return;
...
async function loadHistorico(){
  try{
    ...
    S.historico = res?.fechamentos || [];
    renderHistorico();
  }catch(e){
    $('fecHistorico').innerHTML = `<div class="msg show bad">${esc(e.message || String(e))}</div>`;
  }
}
function kpi(label,value){ return `<div class="fec-kpi"><small>${esc(label)}</small><b>${esc(value)}</b></div>`; }
function renderFechamento(f){
  const sf = f.saldo_final || {};
  const mv = f.resumo_movimentos || {};
  const aud = f.auditoria_resumo || {};
  $('fecKpis').innerHTML = [
    kpi('Ativos', num(sf.equipamentos_ativos)),
    kpi('Em estoque', num(sf.em_estoque)),
    kpi('Com técnico', num(sf.com_tecnico)),
    kpi('Movimentos', num(mv.total_movimentos)),
...
function saldoInicialTexto(si){
  ...
  if(si.tipo === 'sem_fechamento_anterior') return 'Sem fechamento anterior formal';
  if(si.tipo === 'fechamento_anterior') return `Importado do fechamento anterior até ${si.periodo_fim || '-'}`;
...
async function copiarResumo(){
  const f = S.ultimo || S.preview;
  if(!f) return msg('Gere uma prévia ou confirme um fechamento antes de copiar.', 'warn');
  try{ await navigator.clipboard.writeText(resumoWhats(f)); msg('Resumo copiado para WhatsApp.', 'ok'); }catch(e){ window.prompt('Copie o resumo:', resumoWhats(f)); }
}
function pdfText(doc, text, x, y, w=180, lh=5){
  const lines = doc.splitTextToSize(String(text ?? '-'), w);
  lines.forEach(line => { if(y > 280){ doc.addPage(); y = 16; } doc.text(line,x,y); y+=lh; });
  return y;
}
...
function footer(doc){
  ...
  doc.setFontSize(8); doc.setTextColor(110); doc.text('LIKE Estoque • Fechamento operacional • Documento interno',14,287); doc.text(`Página ${p}`,180,287); doc.setTextColor(0);
}
function gerarPdf(f){
  ...
  footer(doc);
  doc.addPage(); y=18;
  doc.setFont('helvetica','bold'); doc.setFontSize(12); doc.text('2. Movimentações do período',14,y); y+=7;
  ...
  footer(doc);
  doc.addPage(); y=18;
  doc.setFont('helvetica','bold'); doc.setFontSize(12); doc.text('4. Auditoria e conferência',14,y); y+=7;
  doc.setFont('helvetica','normal'); doc.setFontSize(9);
  ...
  if((f.divergencias||[]).length){
    y=pdfTable(doc,['Grav.','Categoria','Código','Problema'],(f.divergencias||[]).slice(0,60).map(d=>[d.gravidade,d.categoria,d.codigo,d.problema]),y,[20,42,30,90]);
  }else{
    y=pdfText(doc,'Nenhuma divergência registrada no fechamento.',14,y);
  }
