(function(){
  ...
  function filenameFromArgs(args){
    ...
    if(first && typeof first === 'object' && typeof first.filename === 'string') return first.filename;
  ...
  function install(){
    ...
    if(!jsPDF || !jsPDF.API || typeof jsPDF.API.save !== 'function') return;

    installed = true;
    const originalSave = jsPDF.API.save;

    jsPDF.API.save = function(){
      const filename = filenameFromArgs(arguments);
  ...
  window.LIKE_ESTOQUE_PDF_PRINT_READY = true;
})();
