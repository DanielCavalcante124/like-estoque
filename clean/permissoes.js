...
const S = { ctx:null, perms:{}, perfil:'sem_perfil', busy:false };
...
function css(){
  ...
  s.textContent = `
    ...
    .perm-card b{display:block;color:#0f172a}.perm-card small{color:#64748b}
    .mobile-menu-btn,.sidebar-backdrop{display:none}
    .sidebar-groups{display:grid;gap:8px;margin-top:10px}.side-group{border:1px solid rgba(255,255,255,.08);border-radius:16px;background:rgba(255,255,255,.025);overflow:hidden}
    ...
    .side-group-items{display:grid;gap:4px;padding:6px}.side-group.collapsed .side-group-items{display:none}.side-group-empty{display:none!important}.side-group-items .nav{margin:0!important}
...
function organizeSidebar(){
  ...
  const legacy = side.querySelector('.legacy-link'); const session = side.querySelector('.session-box'); const profile = $('permProfileCard');
  if(legacy && legacy.parentElement === side && side.lastElementChild !== legacy) side.appendChild(legacy);
  if(session && session.parentElement === side && side.lastElementChild !== session) side.appendChild(session);
  if(profile && profile.parentElement === side && side.lastElementChild !== profile) side.appendChild(profile);
}
...
async function loadContext(){
  try{ const ctx = await call('rpc_usuario_contexto_6c', {}); S.ctx = ctx; S.perfil = ctx?.perfil || 'sem_perfil'; S.perms = ctx?.permissoes || {}; document.body.dataset.perfil = S.perfil; renderProfileCard(); applyNavRules(); return ctx; }
  catch(e){ S.ctx = null; S.perms = {}; S.perfil = 'sem_perfil'; document.body.dataset.perfil = 'sem_perfil'; applyNavRules(); return null; }
}
...
function boot(){
  css(); setupSidebarLateral(); organizeSidebar(); protectClicks();
  ...
  setTimeout(loadContext, 1000); setTimeout(loadContext, 2500); setTimeout(applyNavRules, 4500); setInterval(applyNavRules, 5000);
}
