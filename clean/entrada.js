...
function msg(text, type=''){ const el=$('entradaMsg'); if(el){ el.textContent=text; el.className='msg show '+type; } }
...
function inject(){
  ...
  $('entradaReload').onclick=()=>loadEntrada().catch(e=>msg(e.message,'bad')); $('entradaForm').onsubmit=salvarEntrada; $('entradaLimpar').onclick=limparForm; $('entradaModeloSelect').onchange=selecionarModelo; $('entradaBusca').oninput=renderTabela;
...
async function salvarEntrada(ev){ ev.preventDefault(); try{ const p=payload(); msg('Registrando entrada via RPC...','warn'); const result=first(await call('rpc_registrar_entrada_equipamento',{p_tipo:p.tipo,p_marca:p.marca,p_modelo:p.modelo,p_mac:p.mac,p_serial:p.serial,p_local:p.local,p_custo:p.custo,p_observacao:p.obs,p_fornecedor:p.fornecedor,p_nf:p.nf,p_responsavel:p.responsavel,p_client_operation_id:opId()})); msg(`Entrada registrada com sucesso. Código: ${result?.codigo||'gerado'}.`,'ok'); limparForm(false); await loadEntrada(); }catch(e){msg(e.message||String(e),'bad')} }
